...
async function route(){const [page='dashboard',id]=location.hash.slice(1).split('/');$$('#nav a').forEach(a=>a.classList.toggle('active',a.dataset.page===page||(page==='team'&&a.dataset.page==='teams')||(page==='board'&&a.dataset.page==='activities')||(page==='calendar'&&a.dataset.page==='calendar')));setSidebarOpen(false);$('#content').innerHTML=loadingState(t('Loading…'));try{if(page==='dashboard')await dashboard();else if(page==='calendar')await calendarView();else if(page==='activities')await activities();else if(page==='activity'&&id)await activityDetail(id);else if(page==='board'&&id)await taskBoard(id);else if(page==='my-tasks')await myTasks();else if(page==='my-tasks-today')await myTasksToday();else if(page==='teams')await teams();else if(page==='team'&&id)await teamPage(id);else if(page==='people')await people();else if(page==='accounts'&&isExec())await accountsAdmin();else if(page==='documents')await documents();else if(page==='reports'&&canManage())await reports();else if(page==='archive')await archive();else if(page==='settings-deliveries'&&isExec())await settingsDeliveriesPage();else if(page==='settings-templates'&&state.user.is_devops)await settingsTemplatesPage();else if(page==='settings-smtp'&&state.user.is_devops)await settingsSmtpPage();else if(page==='settings-rules'&&state.user.is_devops)await settingsRulesPage();else if(page==='settings-cron'&&state.user.is_devops)await settingsCronPage();else location.hash='dashboard'}catch(e){$('#content').innerHTML=errorState({title:t('Unable to load this page'),detail:t(e.message)})}}
