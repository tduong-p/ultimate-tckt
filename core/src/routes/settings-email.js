...
function createSettingsEmailRoutes(context) {
  const { db, auth, admin, platformAdmin, asyncRoute, one } = context;
  const router = express.Router();

  router.get('/api/admin/email/settings', auth, platformAdmin, asyncRoute(async (_req, res) => {
    res.json({ settings: await getPublicSettings(db) });
  }));

  router.put('/api/admin/email/settings', auth, platformAdmin, asyncRoute(async (req, res) => {
    await updateSettings(db, req.body.settings || {}, req.session.user.id);
    res.json({ ok: true });
  }));

  router.post('/api/admin/email/settings/test-send', auth, platformAdmin, asyncRoute(async (req, res) => {
    try {
  ...
  router.get('/api/admin/email/events', auth, platformAdmin, (_req, res) => {
    res.json({ events: getEventCatalog() });
  });

  router.get('/api/admin/email/templates', auth, platformAdmin, asyncRoute(async (_req, res) => {
    const [rows] = await db.execute('SELECT * FROM email_templates ORDER BY name');
    res.json({ templates: rows });
  }));

  router.post('/api/admin/email/templates', auth, platformAdmin, asyncRoute(async (req, res) => {
  ...
  router.put('/api/admin/email/templates/:id', auth, platformAdmin, asyncRoute(async (req, res) => {
  ...
  router.delete('/api/admin/email/templates/:id', auth, platformAdmin, asyncRoute(async (req, res) => {
  ...
  router.get('/api/admin/email/rules', auth, platformAdmin, asyncRoute(async (_req, res) => {
    const [rows] = await db.execute('SELECT * FROM email_rules ORDER BY event_key, priority DESC');
    res.json({ rules: rows });
  }));

  router.post('/api/admin/email/rules', auth, platformAdmin, asyncRoute(async (req, res) => {
  ...
  router.put('/api/admin/email/rules/:id', auth, platformAdmin, asyncRoute(async (req, res) => {
  ...
  router.delete('/api/admin/email/rules/:id', auth, platformAdmin, asyncRoute(async (req, res) => {
    await db.execute('DELETE FROM email_rules WHERE id=?', [req.params.id]);
    res.json({ ok: true });
  }));

  router.post('/api/admin/email/rules/:id/simulate', auth, platformAdmin, asyncRoute(async (req, res) => {
    const [rows] = await db.execute('SELECT * FROM email_rules WHERE id=?', [req.params.id]);
    const rule = one(rows);
  ...
  router.patch('/api/admin/email/rules/:id/activate', auth, platformAdmin, asyncRoute(async (req, res) => {
    const [rows] = await db.execute('SELECT * FROM email_rules WHERE id=?', [req.params.id]);
    const rule = one(rows);
    if (!rule) return res.status(404).json({ error: 'Không tìm thấy rule.' });
    const event = getEvent(rule.event_key);
  ...
  router.patch('/api/admin/email/rules/:id/deactivate', auth, platformAdmin, asyncRoute(async (req, res) => {
