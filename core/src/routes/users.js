...
router.get('/api/people',auth,asyncRoute(async(req,res)=>{
  ...
  if(!isExecutive(req.session.user)){
  ...
  for(const row of rows)row.can_manage=await canManageUser(req.session.user,row.id);
  res.json(rows)
}));
...
router.post('/api/users', auth, manager, asyncRoute(async (req, res) => {
  ...
  const teamIds = ids(req.body.team_ids), allowedTeams = await managedTeamIds(req.session.user);
  ...
  if (isLeadership(req.session.user) && teamIds.some(id => !allowedTeams.includes(id))) {
  ...
  try {
    ...
    await conn.commit();
    await syncTcktMembershipFromRole(db, result.insertId);
