...
const { BACKFILL_MARKER } = require('../config/migrate-units');
...
router.get('/api/health',asyncRoute(async(_req,res)=>{try{await db.query('SELECT 1');const [marker]=await db.execute('SELECT 1 FROM platform_migrations WHERE name=?',[BACKFILL_MARKER]);if(!marker.length)return res.status(503).json({status:'migration_incomplete'});res.json({status:'ok'})}catch(error){logger.error('Database health check failed.',error);throw error}}));
