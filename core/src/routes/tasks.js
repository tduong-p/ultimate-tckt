...
function createTaskRoutes(context) {
  ...
  async function canTouchTask(context, req, taskId) {
    ...
    const [rows] = await db.execute('SELECT t.*,a.title activity_title,EXISTS(SELECT 1 FROM task_assignees ta WHERE ta.task_id=t.id AND ta.user_id=?) assigned_to_me FROM tasks t JOIN activities a ON a.id=t.activity_id WHERE t.id=?', [req.actor.id, taskId]);
    const task = rows[0] || null;
    if (!task) return { task: null, allowed: false };
    const manages = await canManageTeam(req.actor, task.team_id);
...
router.post('/api/tasks/:id/acknowledge', auth, asyncRoute(async (req, res) => {
  ...
  if (!result.affectedRows) {
    const [rows] = await db.execute('SELECT 1 FROM task_assignees WHERE task_id=? AND user_id=?', [req.params.id, req.actor.id]);
...
router.patch('/api/tasks/:id/status', auth, asyncRoute(async (req, res) => {
  const [rows] = await db.execute('SELECT t.*,EXISTS(SELECT 1 FROM task_assignees ta WHERE ta.task_id=t.id AND ta.user_id=?) assigned_to_me FROM tasks t WHERE t.id=?', [req.actor.id, req.params.id]);
  const task = one(rows);
...
router.patch('/api/tasks/:id/checklist/:itemId', auth, asyncRoute(async (req, res) => {
  ...
  const [result] = await db.execute(
    ...
    [isDone, isDone ? req.actor.id : null, isDone ? new Date() : null, req.params.itemId, req.params.id]
  );
...
router.post('/api/tasks/:id/submit-review', auth, taskUpload.single('file'), asyncRoute(async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      for (const reviewer of reviewers) emailEvents.emit(db, 'task.submitted_for_review', { reviewer: { id: reviewer.id, name: reviewer.name, email: reviewer.email }, task: { id: task.id, title: task.title, activity_title: task.activity_title }, submitted_by: req.actor.name, url: activityUrl(task.activity_id) }, { logger });
...
router.post('/api/tasks/:id/review', auth, asyncRoute(async (req, res) => {
  ...
  if (!task) return res.status(404).json({ error: 'Không tìm thấy công việc.' });
  if (!(await canReviewTask(req.actor, task))) return res.status(403).json({ error: 'Bạn không thể nghiệm thu công việc này.' });
  ...
  await db.execute(`UPDATE tasks SET status=?,reviewed_by=?,reviewed_at=NOW(),review_feedback=?${completeSql} WHERE id=?`, [nextStatus, req.actor.id, feedback || null, req.params.id]);
...
router.post('/api/tasks/:id/cancel', auth, asyncRoute(async (req, res) => {
  ...
  const manages = await canManageTeam(req.actor, task.team_id);
  const isExec = isExecutive && isExecutive(req.actor);
  const isSelfLoggedOwner = Boolean(task.is_self_logged && (Number(task.assigned_by) === Number(req.actor.id) || !!task.assigned_to_me));
  ...
  const cancelNote = isSelfLoggedOwner
    ? `${req.actor.name} đã rút lại công việc tự ghi nhận này.`
    : `${req.actor.name} đã hủy công việc này.`;
  await db.execute(
    "INSERT INTO updates(activity_id,task_id,user_id,body,kind) VALUES(?,?,?,?,'progress')",
    [task.activity_id, task.id, req.actor.id, cancelNote]
  );
