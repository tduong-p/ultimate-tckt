...
router.post('/api/activities/:id/submit', auth, managerOrEventLead, asyncRoute(async (req, res) => {
  ...
  res.json({ ok: true });
  try {
    emailEvents.emit(db, 'activity.proposed', { activity, proposed_by: req.actor.name, url: activityUrl(req.params.id) }, { logger });
...
router.post('/api/activities/:id/updates',auth,asyncRoute(async(req,res)=>{
  ...
  if(taskId)try{
    ...
    if(task)for(const owner of owners){
      emailEvents.emit(db,'task.response_posted',{owner:{id:owner.id,name:owner.name,email:owner.email},task:{id:task.id,title:task.title,activity_title:task.activity_title},responded_by:req.actor.name,response_kind_label:responseKindLabel,response_body:responseBodyTrimmed.length>500?`${responseBodyTrimmed.slice(0,497)}...`:responseBodyTrimmed,url:activityUrl(task.activity_id)},{logger});
    }
...
router.post('/api/activities/:id/tasks', auth, manager, asyncRoute(async (req, res) => {
  ...
  if (!(await canManageTeam(req.actor, team_id))) return res.status(403).json({ error: 'Bạn không thể giao việc cho ban này.' });
  ...
  try {
    ...
    for (const assignedUser of users) {
      emailEvents.emit(db, 'task.assigned', { user: { id: assignedUser.id, name: assignedUser.name, email: assignedUser.email }, task, assigned_by: req.actor.name, url: activityUrl(task.activity_id) }, { logger });
      await db.execute(
...
router.post('/api/activities/:id/log-task', auth, taskUpload.single('file'), asyncRoute(async (req, res) => {
  ...
  try {
    ...
    for (const reviewer of reviewers) {
      emailEvents.emit(db, 'task.submitted_for_review', { reviewer: { id: reviewer.id, name: reviewer.name, email: reviewer.email }, task: { id: taskObj.id, title: taskObj.title, activity_title: taskObj.activity_title }, submitted_by: req.actor.name, url: activityUrl(activityId) }, { logger });
      await db.execute(
