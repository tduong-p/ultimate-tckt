...
function createSettingsCronRoutes(context) {
  const { db, auth, devops, asyncRoute, one, logger } = context;
  const router = express.Router();

  router.get('/api/admin/cron/handlers', auth, devops, (_req, res) => {
    res.json({ handlers: getRegisteredHandlerKeys() });
  });

  router.get('/api/admin/cron/jobs', auth, devops, asyncRoute(async (_req, res) => {
    const [rows] = await db.execute('SELECT * FROM cron_jobs ORDER BY name');
    res.json({ jobs: rows });
  }));

  router.post('/api/admin/cron/jobs', auth, devops, asyncRoute(async (req, res) => {
  ...
  router.put('/api/admin/cron/jobs/:id', auth, devops, asyncRoute(async (req, res) => {
  ...
  router.delete('/api/admin/cron/jobs/:id', auth, devops, asyncRoute(async (req, res) => {
    unscheduleJob(Number(req.params.id));
    await db.execute('DELETE FROM cron_jobs WHERE id=?', [req.params.id]);
    res.json({ ok: true });
  }));

  router.patch('/api/admin/cron/jobs/:id/activate', auth, devops, asyncRoute(async (req, res) => {
  ...
  router.patch('/api/admin/cron/jobs/:id/deactivate', auth, devops, asyncRoute(async (req, res) => {
  ...
  router.post('/api/admin/cron/jobs/:id/run-now', auth, devops, asyncRoute(async (req, res) => {
    const outcome = await runJobById(db, logger, req.params.id);
    res.json(outcome);
  }));

  router.get('/api/admin/cron/jobs/:id/runs', auth, devops, asyncRoute(async (req, res) => {
