...
5. **Compose**: thêm service mới vào **cả hai** file `infra/compose/docker-compose.staging.yml` và `docker-compose.production.yml` — service tên `<id>`, `<id>-db` nếu có DB riêng, volume riêng theo quy ước `<id>_<phần dữ liệu>` (xem ví dụ `ctd_mysql`, `ctd_documents`).
...
| 1.2 | 2026-09-26 | CTD chuyển sang MySQL: cập nhật tên volume ví dụ ctd_mysql | DYC |
